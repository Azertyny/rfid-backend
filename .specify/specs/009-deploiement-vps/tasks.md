...
**Tests**: No new automated test suite is requested. US1 _is_ a test fix (FR-002) and its tasks include the run that
...
- [x] T009 [US1] **(operator)** Push the branch, open the PR to `dev`, check the `ci / test` run is green and appears on the PR; then in GitHub settings add branch protection on `dev` and `main` requiring status check `test` (FR-004); document this setting in the "GitHub setup" section of `deploy/INSTALL.md` (created in T021)
...
- [x] T025 [US3] **(operator)** Run quickstart §4 steps 5–8 on the VPS and note timings against SC-005 and SC-006 in `.specify/specs/009-deploiement-vps/quickstart.md` (a "Results" line under §4)
...
- [x] T037 [P] **(operator)** Delete the superseded remote branch `feature/mep_vps` once the VPS runs from this feature (research R11); record it in the PR description
...
- [x] T038 In `deploy/vps/deploy.sh`, when the 120 s health wait times out, tell the two failure causes apart: if `app_healthy` fails, print `compose logs --tail 100 app` and fail 6 with "application not healthy"; if the app is healthy but `site_healthy` fails, print `compose logs --tail 100 web` and fail 6 with "public HTTPS check on https://$site failed: check that the DNS record points to this VPS and that ports 80/443 are open (certificate not obtained yet?)"; keep the "redeploy $previous if needed" hint in both; update the exit-6 row of `.specify/specs/009-deploiement-vps/contracts/vps-scripts.md` and the "Deploy fails with exit 6" line of `deploy/INSTALL.md` accordingly per spec edge case "nom de domaine ne pointe pas encore vers le VPS" (partial)
- [x] T039 In `deploy/vps/backup.sh`, exit non-zero with the message "nothing deployed yet: /opt/vegelink/compose.yml is missing" before calling `docker compose` when that file does not exist; in `deploy/INSTALL.md` "First install", add that until the first deployment the daily backup timer and the `backup-check` workflow fail as expected (the workflow can be disabled from the Actions tab until then) per FR-017b (partial)
- [x] T040 Align the design documents with the implemented code: add `BACKUP_REMOTE` (rclone remote + bucket, not a secret) to the "Environnement" table of `.specify/specs/009-deploiement-vps/data-model.md` and to research R8 in `.specify/specs/009-deploiement-vps/research.md` (replacing "rclone remote `backup`" as the upload target); state in `.specify/specs/009-deploiement-vps/contracts/pipelines.md` (`ci.yml`, job `test`) that shellcheck runs from the pinned image `koalaman/shellcheck:v0.11.0` and that `docker compose config` validates `deploy/compose.yml` per plan: research R8/R9 (contradicts)

---

## Phase 10: Convergence

- [x] T041 In `deploy/vps/deploy.sh`, run `compose up -d --remove-orphans` under an explicit check: on failure print `compose logs --tail 50 web` and `compose logs --tail 50 app`, and `fail 8 "containers could not start (port 80/443 already in use? see sudo ss -ltnp)"`; add exit code 8 to `.specify/specs/009-deploiement-vps/contracts/vps-scripts.md` and to the exit-code list and "Troubleshooting" of `deploy/INSTALL.md` per FR-012 (partial)
- [x] T042 In `.github/workflows/ci.yml`, add `provenance: false` to both `docker/build-push-action` steps so each version is a single manifest with no untagged attestation versions (deleting one broke `docker manifest inspect` in `deploy.yml` for `main-b2373bf`); note in `deploy/INSTALL.md` "GitHub setup" that only whole versions (tags) may be deleted from the packages per plan: research R3 (unrequested)
- [x] T043 In `deploy/vps/bootstrap.sh`, after the firewall step, warn (without failing) when ports 80 or 443 are held by a process other than `docker-proxy` (`ss -ltnpH '( sport = :80 or sport = :443 )'`), naming the process and suggesting `docker compose down` for an old stack or `systemctl disable --now nginx|apache2`; in `deploy/INSTALL.md` "First install", add a step to stop any previous deployment (old `deploy-*` containers, host nginx/Apache) and a "port 80/443 already in use" troubleshooting entry per FR-019 and US2/AC1 (partial)
- [x] T044 In `deploy/INSTALL.md` "Backups", document the temporary local setting `BACKUP_REMOTE=/var/backups/vegelink` (no rclone remote needed; pre-deploy and daily backups still run and `restore.sh` works), its limits (lost with the VPS, no automatic expiry, `backup-check` must stay disabled) and the switch to the external bucket, stating that FR-017 is met only with the bucket per FR-017 and FR-019 (partial)
