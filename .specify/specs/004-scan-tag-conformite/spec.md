...
7. **Given** ce même `Record` est conforme, **When** le scan répété déclare `isCompliant: false`, **Then** le `Record` passe à non conforme et la réponse porte `isCompliant: false` ; un scan répété conforme ne le remet jamais à conforme (FR-008). Depuis la spec `005` (FR-006), cela ne vaut que si aucun Opérateur n'a modifié ce `Record` ; sinon sa conformité reste inchangée.
